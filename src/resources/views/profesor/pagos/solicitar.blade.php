...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Solicitar Pagos</h1>
            <p class="text-gray-600">Crea solicitudes de pago para grupos o alumnos específicos</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        <form action="{{ route('pagos.solicitar.store') }}" method="POST" class="p-8 space-y-8">
            @csrf

            <div class="grid grid-cols-1 gap-6" x-data="alumnoSelector({
                selectedAlumnos: {{ json_encode((array)old('alumnos', [])) }},
                selectedAlumnosData: [],
                selectedGrupos: {{ json_encode((array)old('grupos', [])) }},
                generalGroupId: '{{ $generalGroupId }}'
            })" x-init="
                @if(old('alumnos'))
                    const oldIds = {{ json_encode(old('alumnos')) }};
                    fetch(`{{ route('alumnos.search') }}?ids=${oldIds.join(',')}`)
                        .then(res => res.json())
                        .then(data => { selectedAlumnosData = data; });
                @endif
            ">
                <div class="space-y-4">
                            ...
                            <div @click="toggleGrupo('{{ $generalGroup->id }}')"
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500">Solicitar a todos los alumnos del equipo</p>
                                    </div>
                                </div>
                ...
                <div class="space-y-3" x-show="!isGeneralSelected()" x-transition>
                    <div class="flex items-center justify-between">
                        <label class="text-sm font-semibold text-gray-700 flex items-center gap-2">
                            <i class="fas fa-user text-blue-600"></i>
                            Solicitar a Alumnos Individuales
                        </label>
